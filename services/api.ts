...
function runInterceptorAI() {
    ...
    const threatsToIntercept = ufos.filter(ufo => {
        ...
        const isAlreadyTargeted = Object.values(drones).some(d => d.interceptTargetId === ufo.id);
        if (isAlreadyTargeted) return false;
            ...
            if (approved && drone) {
                ...
                switch (request.action) {
                    ...
                    case AIAction.FLY_INTO_THREAT:
                        // This action now directly approves elimination for an interceptor that was auto-dispatched
                        if (drone.droneType === 'Interceptor' && (drone.interceptTargetId || drone.interceptThreatId)) {
                             drone.isEliminationApproved = true;
                        } 
                        // This handles the case where an interceptor was manually sent to a mission target
                        else if (drone.droneType === 'Interceptor' && drone.mission_target) {
                            drone.status = DroneStatus.AI_OVERRIDE;
                            drone.isEliminationApproved = true;
                        }
                        // Fallback for other drone types or weird states if a threatId is present
                        else {
                    ...
                    case AIAction.FLY_INTO_TARGET:
                        drone.isEliminationApproved = true;
                        drone.status = DroneStatus.AI_OVERRIDE;
                        // FIX: If missionTargetId is not set (e.g., from a manual mission), 
                        // find a nearby threat/UFO at the mission coordinates and assign its ID.
                        // This ensures the target is correctly removed from the simulation upon elimination.
...
setInterval(() => {
    ...
    // Assault drones are now only dispatched by operator command (manual mission or approving an AI target designation).
    // The autonomous runAssaultAI() function has been removed.
    runReconAI();
